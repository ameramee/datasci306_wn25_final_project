...
ui <- fluidPage(
  titlePanel("IMDb Interactive Explorer"),
  sidebarLayout(
    sidebarPanel(
      selectInput(
        "genre", "Select Genre(s)",
        choices  = sort(unique(title_genre$genres)),
        selected = c("Drama","Comedy"),
        multiple = TRUE
      ),
      sliderInput(
        "yearRange", "Release Year Range:",
        min   = min(title_genre$startYear),
        max   = max(title_genre$startYear),
        value = c(2000,2020), sep = ""
      ),
      sliderInput(
        "minVotes", "Minimum Number of Votes:",
        min   = 0,
        max   = max(title_ratings$numVotes, na.rm=TRUE),
        value = 1000, step = 100
      ),
      selectInput(
        "plotType", "Plot Type:",
        choices = c("Runtime Distribution","Ratings vs Votes"),
        selected = "Runtime Distribution"
      )
    ),
    mainPanel(
...
server <- function(input, output, session) {
  # Reactive filtered data by genre and year
  filtered <- reactive({
    req(input$genre)
    title_genre %>%
      filter(
        genres    %in% input$genre,
  ...
  output$mainPlot <- renderPlotly({
    df <- filtered()
    if (input$plotType == "Runtime Distribution") {
      p <- ggplot(df, aes(x = runtimeMinutes, fill = genres,
                           text = paste0(genres,": ",runtimeMinutes," min"))) +
        geom_density(alpha = 0.4) +
        labs(title = "Runtime Distribution by Genre",
             x = "Runtime (minutes)", y = "Density")
      ...
      df2 <- title_basics %>%
        ...
        filter(
          ...
          startYear >= input$yearRange[1],
          startYear <= input$yearRange[2],
          numVotes  >= input$minVotes
        )
      p2 <- ggplot(df2, aes(x = numVotes, y = averageRating,
                             color = genres,
                             text = paste0(primaryTitle," (",startYear,")"))) +
  ...
  output$dataTable <- renderDT({
    df_tab <- filtered() %>%
      left_join(title_basics %>% select(tconst, primaryTitle, startYear), by="tconst") %>%
      left_join(title_ratings %>% select(tconst, averageRating, numVotes), by="tconst") %>%
      select(primaryTitle, startYear, genres, runtimeMinutes, averageRating, numVotes) %>%
      distinct() %>%
      arrange(desc(numVotes))
    datatable(df_tab, options = list(pageLength = 10, scrollX = TRUE))
  })
}
...
ui2 <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput(
        "categories", "Select Job Category(ies)",
        choices  = sort(unique(title_principals$category)),
        multiple = TRUE
      )
    ),
    mainPanel(
      DTOutput("principalsTable")
    )
  )
)
...
server2 <- function(input, output, session) {
  ...
  output$principalsTable <- renderDT({
    datatable(
      filtered_principals(),
...
# To run Application 2 within the same document, call:
